...
class VirusTotalManager @Inject constructor(
    ...
    suspend fun scanFilesWithVirusTotal(
        ...
        for ((index, file) in files.withIndex()) {
            ...
            if (!isActive) return
            // Progress update before scan
            val progressBefore = (index * 100) / totalFiles
            onProgress?.invoke(progressBefore, "Scanning ${file.name}")

            val result = virusTotalRepo.scanFile(file)
            results.add(result)

            // Progress update after scan
            val progressAfter = ((index + 1) * 100) / totalFiles
            onProgress?.invoke(progressAfter, "Scanned ${file.name}")
        }

        // Agar cancel nahi hua tabhi save karo
        if (isCancelled?.invoke() != true) {
            saveResultsToDownloads(results, context)
    ...
    suspend fun saveResultsToDownloads(results: List<VirusTotalResult>, context: Context) {
        withContext(Dispatchers.IO) {
            try {
                ...
                results.forEach { r ->
                    ...
                    var fileOut = File(scanFolder, "${safeFileName}_scan.html")

                    // ✅ Agar file already exist hai to timestamp add kar do
                    if (fileOut.exists()) {
                        val timestamp =
                            SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(Date())
                        fileOut = File(scanFolder, "${safeFileName}_scan_$timestamp.html")
                    }
                    ...
                    <body>
                        ...
                        <div class="summary-stats">
                            <div class="stat-box"><div class="stat-value stat-malicious">${r.malicious}</div><div>Malicious</div></div>
                            <div class="stat-box"><div class="stat-value stat-harmless">${r.harmless}</div><div>Harmless</div></div>
                            <div class="stat-box"><div class="stat-value stat-suspicious">${r.suspicious}</div><div>Suspicious</div></div>
                            <div class="stat-box"><div class="stat-value stat-undetected">${r.undetected}</div><div>Undetected</div></div>
                            <div class="stat-box"><div class="stat-value stat-timeout">${r.timeout}</div><div>Timeout</div></div>
                        </div>
                ...
                """.trimIndent()
                    ...
                    mediaScanIntent.data = Uri.fromFile(fileOut)
                    context.sendBroadcast(mediaScanIntent)
                }

            } catch (e: Exception) {
                Log.e("VirusTotalManager", "Error saving results: ${e.message}", e)
                throw e
            }
        }
    }
